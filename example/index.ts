...
compiled.templates.forEach(template => {
  ...
  switch(template.operation) {
    case "create":
      console.log(`Would create file at ${template.path} with content: ${(template as any).template.substring(0, 30)}...`);
      break;
    
    case "edit":
      console.log(`Would edit file at ${template.path} with ${(template as any).replacements.length} replacements`);
      (template as any).replacements.forEach((replacement: any, index: number) => {
        console.log(`  ${index+1}. Replace "${replacement.oldString}" with "${replacement.newString}"`);
      });
      break;
    
    case "append":
      console.log(`Would append to file at ${template.path} with content: ${(template as any).content.substring(0, 30)}...`);
      break;
